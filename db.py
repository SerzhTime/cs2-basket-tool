...
def init_db(*, migrate: bool = True, maintenance: bool = True) -> None:
    ...
    with connect() as con:
        if migrate:
            ...
            ensure_basket_transition_schema(con)
...
def _schema_sql(backend: str | None = None) -> str:
    ...
    if is_postgres:
        return """
            ...
            CREATE TABLE IF NOT EXISTS basket_transitions (
                transition_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                snapshot_id INTEGER NOT NULL,
                snapshot_timestamp TEXT NOT NULL,
                period_start TEXT NOT NULL,
                offset_amount DOUBLE PRECISION NOT NULL,
                added_item_ids_json TEXT NOT NULL,
                contribution_marketplace TEXT NOT NULL,
...
def ensure_basket_transition_schema(con: DbConnection) -> None:
    if _table_column_names(con, "basket_transitions"):
        return
    ...
    con.execute(
        ...
        CREATE TABLE IF NOT EXISTS basket_transitions (
            ...
            snapshot_timestamp TEXT NOT NULL,
            period_start TEXT NOT NULL,
            offset_amount {numeric_type} NOT NULL,
            added_item_ids_json TEXT NOT NULL,
            contribution_marketplace TEXT NOT NULL,
            computed_at TEXT NOT NULL
        )
...
def _sync_sqlite_to_postgres_once() -> dict[str, int]:
    ...
    try:
        with connect_postgres() as target:
            ...
            _sync_basket_transitions_to_postgres(source, target, counts)
...
def _sync_basket_transitions_to_postgres(source: sqlite3.Connection, target: DbConnection, counts: dict[str, int]) -> None:
    rows = source.execute("SELECT * FROM basket_transitions ORDER BY transition_id").fetchall()
    if not rows:
        return
    target.execute("DELETE FROM basket_transitions")
    target.executemany(
        """
        INSERT INTO basket_transitions (
            transition_id, snapshot_id, snapshot_timestamp, period_start, offset_amount,
            added_item_ids_json, contribution_marketplace, computed_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(transition_id) DO UPDATE SET
            ...
            added_item_ids_json=excluded.added_item_ids_json,
            contribution_marketplace=excluded.contribution_marketplace,
            computed_at=excluded.computed_at
        """,
        [tuple(row) for row in rows],
    )
    counts["basket_transitions"] = len(rows)
...
def record_basket_transition(
    *,
    snapshot_id: int,
    snapshot_timestamp: str,
    offset_amount: float,
    added_item_ids: list[int],
    contribution_marketplace: str = BASELINE_MARKETPLACE,
) -> None:
    with connect() as con:
        con.execute(
            """
            INSERT INTO basket_transitions (
                snapshot_id, snapshot_timestamp, period_start, offset_amount,
                added_item_ids_json, contribution_marketplace, computed_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                snapshot_id,
                snapshot_timestamp,
                _history_period_start(snapshot_timestamp),
                offset_amount,
                json.dumps(sorted(added_item_ids)),
...
def basket_transitions() -> list[dict]:
    with connect() as con:
        return [dict(row) for row in con.execute(
            "SELECT * FROM basket_transitions ORDER BY snapshot_timestamp, transition_id"
        ).fetchall()]
...
def _apply_basket_transitions_to_history(
    combined: dict[tuple[str, str], dict],
    raw_grouped: dict[tuple[str, str], list[dict]],
    transitions: list[dict],
) -> None:
    for key, samples in raw_grouped.items():
        period_start, _marketplace = key
        transitions_in_period = [
            transition
            for transition in transitions
            if _history_period_start(transition["snapshot_timestamp"]) == period_start
        ]
        if transitions_in_period:
            cutoff = max(transition["snapshot_timestamp"] for transition in transitions_in_period)
        ...
        values = [
            ...
            + sum(
                float(transition["offset_amount"])
                for transition in transitions
                if sample["timestamp"] < transition["snapshot_timestamp"]
            )
            for sample in samples
        ]
        row = combined[key]
        row["average_total_cost"] = sum(values) / len(values)
        row["min_total_cost"] = min(values)
        row["max_total_cost"] = max(values)
        row["sample_count"] = len(values)
        row["first_sample_at"] = samples[0]["timestamp"]
        row["last_sample_at"] = samples[-1]["timestamp"]

    for key, row in combined.items():
        if key in raw_grouped:
            continue
        offset = sum(
            float(transition["offset_amount"])
            for transition in transitions
            if row["period_start"] < transition["period_start"]
        )
        if offset:
            row["average_total_cost"] += offset
            row["min_total_cost"] += offset
            row["max_total_cost"] += offset
def history_daily_display_totals(since_iso: str | None = None) -> list[dict]:
    ...
    transitions = basket_transitions()
    for key, samples in raw_grouped.items():
        if key in combined:
            continue
    ...
    _apply_basket_transitions_to_history(combined, raw_grouped, transitions)

...
try:
    ...
    # Streamlit can retain adapters.base while hot-reloading app.py after both
    # files changed. Reload the dependency once; fresh processes use the
    # normal import path above.
    from adapters import base as adapter_base

    safe_error_details = importlib.reload(adapter_base).safe_error_details
...
def load_text_asset(name: str) -> str:
    return (db.APP_DIR / "assets" / name).read_text(encoding="utf-8")


def inject_app_styles() -> None:
    st.markdown(f"<style>{load_text_asset('app.css')}</style>", unsafe_allow_html=True)


def run_parent_script(name: str) -> None:
    components.html(f"<script>{load_text_asset(name)}</script>", height=0)
...
def render_comparison_scrollbar_decorator() -> None:
    run_parent_script('comparison_scrollbars.js')


def render_readonly_table_scrollbars() -> None:
    run_parent_script('readonly_table_scrollbars.js')
